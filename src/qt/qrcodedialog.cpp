...
{
    QString ret = QString("hongkongcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HKC unit input in lnReqAmount, we generate the URI with HKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(HongKongCoinUnits::format(HongKongCoinUnits::HKC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
